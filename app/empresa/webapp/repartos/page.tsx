...
export default async function RepartosPage() {
  ...
  if (!isAllowed(userRole, ["Admin", "Supervisor", "Closer", "Cambaceador", "Repartidor", "Developer"])) {
          ...
          {canEdit && (
            <>
              ...
              <Link 
                ...
                className="flex items-center justify-center gap-1 md:gap-2 px-2.5 md:px-5 py-2 md:py-2.5 bg-slate-800 text-slate-200 border border-slate-700 rounded-xl hover:bg-slate-700 transition-all text-[11px] md:text-sm cursor-pointer whitespace-nowrap" 
                title="Configurar Zonas de Reparto"
              >
                <span className="material-symbols-outlined text-[16px] md:text-lg">map</span>
                Zonas
              </Link>
            </>
          )}
          <Link href="/empresa/webapp" className="flex items-center justify-center px-3 py-2 md:px-4 md:py-2.5 bg-slate-800 text-slate-400 border border-slate-700 rounded-xl hover:bg-slate-700 hover:text-white transition-all cursor-pointer" title="Volver al Inicio">
            <span className="material-symbols-outlined text-lg md:text-xl">home</span>
          </Link>
        </div>
      </header>
